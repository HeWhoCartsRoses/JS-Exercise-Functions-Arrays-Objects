...
function getLastCarInfo(arg) {
  return `this is a ${arg[arg.length - 1].car_make} ${arg[arg.length - 1].car_model}`
}
//console.log(getLastCarInfo(id: 1, car_make: "Lincoln", car_model: "Navigator", car_year: 2009));
...
*/
function getOlderCars(inv, year) {

  const max = [];
  for (var i = 0; i < inv.length; i++)
    if (inv[i].car_year <= year) {
      max.push(inv[i]);
    }
  return max;
}
